...
from chargax.algorithms import build_ppo_pid_trainer
from chargax.algorithms import ppo_pid
...
if __name__ == "__main__":
    ...
    groupname = f"{groupname}_{env_parameters_str}_ppo_pid"
    
    tags = [RUNTAG, "ppo-pid"] if RUNTAG else ["ppo-pid"]
    wandb.init(project="chargax", config=merged_config, group=groupname, tags=tags, dir="./wandb")
    
    ppo_pid._pbar = tqdm(total=config.num_iterations, desc="PPO-PID Training", unit="iter")
    c_time = time.time()
    ...
    print(f"最终拉格朗日乘子: {trained_agent.lagrangian_multiplier:.4f}")
